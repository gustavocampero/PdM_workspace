...
sd_err_t SD_init()
{
	...
	if(!send_CMD0()) return SD_ERROR;

	sd_dummy();

	if(!send_CMD8()) return SD_ERROR;
...
sd_err_t SD_read(uint32_t block_addr, uint8_t *buffer)
{
	...
	if (sd_send_cmd(CMD17, block_addr, 0x01) != 0x00)
	{
		return SD_ERROR;
	}
...
sd_err_t SD_write(uint32_t block_addr, const uint8_t *buffer)
{
	...
	cs_Low();
	uint8_t ret = sd_send_cmd(CMD24, block_addr, 0x01);
	if (ret != 0x00) {
		cs_High();
		return false;
	}
...
static sd_err_t send_CMD0()
{
	...
	for (int i = 0; i < tries; i++) {
		uint8_t resp = sd_send_cmd(CMD0, 0, 0x95);
...
static sd_err_t send_CMD8()
{
	...
	if(sd_send_cmd(CMD8, 0x1AA, 0x87) == 0x01)
	{
		for (int i = 0; i < 4; i++)
		{
			sd_TransmitReceive(&dummy, &r7[i], 1);
		}
...
static sd_err_t send_ACMD41(bool force)
{
	...
	do
	{
		sd_send_cmd(CMD55, 0, 0x01); // no es necesario argumento

		resp = sd_send_cmd(ACMD41, 0x40000000, 0x01); // 0x40000000 -> bit HCS (Host Capacity Support) indica soporte a SDHC/SDXC.

		sd_dummy();

		tries--;
		if(tries <= 0) return SD_TIMEOUT;
